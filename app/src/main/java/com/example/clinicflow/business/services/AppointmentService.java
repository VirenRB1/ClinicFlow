...
public class AppointmentService {
    ...
    public List<Appointment> getPastAppointmentsForPatient(String patientEmail) {
        ...
        for (int i = 0; i < allAppointments.size(); i++) {
            ...
            if ("Completed".equalsIgnoreCase(appt.getStatus())
                    || (isPastTime && !"Cancelled".equalsIgnoreCase(appt.getStatus()))) {
                past.add(appt);
            }
        }
    ...
    /**
     * Checks if a specific date and time are in the future relative to the current
     * moment.
     * 
    ...
    public List<TimeSlot> getAvailableTimeSlots(String doctorEmail, LocalDate date) {
        ...
        List<DoctorAvailability> doctorAvailabilities = userRepository.getDoctorAvailability(doctorEmail,
     ...
     * @throws ValidationExceptions.ValidationException If the date is invalid or if
     *                                                  there's a conflict.
     */
    ...
    /**
     * Generates all possible time slots based on doctor availability and existing
     * appointments.
     * 
    ...
    /**
     * Checks if a given time range falls within any of the doctor's availability
     * windows.
     * 
    ...
    /**
     * Checks if a requested time slot is free of existing, non-cancelled
     * appointments.
     * 
