...
//
//        // If inputs are invalid, treat as conflict (safer than allowing bad bookings)
...
//        }
//
//        if (hasConflictingAppointment(doctorsEmail, startDateTime, endDateTime, existingAppointments)) {
//            throw new IllegalStateException("The doctor is not available during the requested time slot.");
//        }
//
...
//    }
//
//   public List<TimeSlot> filterAppointmentTimeSlots(List<TimeSlot> timeSlots, List<Appointment> existingAppointments) {
...
//            boolean sameDoctor = slot.getDoctorsEmail().equals(appointment.getDoctorsEmail());
//
//            boolean appointmentActive = appointment.getStatus() != AppointmentStatus.CANCELLED;
//
//            if (sameDoctor && appointmentActive) {boolean overlaps = slot.getStartDateTime().isBefore(appointment.getEndDateTime()) &&
